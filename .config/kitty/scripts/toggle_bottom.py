# type: ignore
"""
Kitty kitten: toggle bottom (BTM_*) pane collapse/expand.
...
def handle_result(args, data, target_window_id, boss: Boss):
    ...
    btm_windows = [(wid, title, rows, cols) for wid, title, rows, cols in _windows_in_active_tab(boss) if title.startswith("BTM_")]
    if not btm_windows:
        return

    # Prefer focused, else largest id
    focused_id = getattr(boss.active_window, "id", None) if getattr(boss, "active_window", None) else None
    target = None
    for wid, title, rows, cols in btm_windows:
        if focused_id is not None and wid == focused_id:
            target = (wid, rows, cols)
            break
    if target is None:
        target = max(btm_windows, key=lambda x: x[0])
    ...
    if not collapsed:
        ...
        restore_to = int(state.get(key, 10))
        increment = max(restore_to - rows, 1)
